...
buffer=inputarray.loc[idx].buffer
...
if __name__ == '__main__':
    ...
    if (os.path.exists("%s/best_model.zip" % savepath)):
        ...
        model = ACER(policy, env, gamma=gamma, n_steps=episodetimesteps, learning_rate=LR,  buffer_size=buffer,  verbose=1)
        ...
        model = ACER(policy, env, gamma=gamma, n_steps=episodetimesteps, learning_rate=LR,  buffer_size=buffer,  verbose=1)#, tensorboard_log=scenario)
