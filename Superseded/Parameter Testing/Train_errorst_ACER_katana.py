...
from tools.loadsaveBMenv_errorst import environment
...
inputarray=pd.read_csv('jobarrays/ACER_katana_errors_job_input.csv')
...
scenario=str(f'{trialv}t_{inputfile_s}_t{test}_lr{LR_s}_g{gamma_s}_p{penaltyscalar}')    
...
def make_env(x,y,z, rank, seed=0):
    ...
    def _init():
        ...
        env = environment(x, y, z, gamma, turnspc, penaltyscalar, policyname)
        env.seed(seed + rank)
        return env
