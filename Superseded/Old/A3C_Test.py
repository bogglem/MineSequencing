...
from keras.layers import Dense, Input,Conv3D, Dropout, Flatten
...
class A3CAgent:
    ...
    def build_model(self):
        ...
        critic=Sequential()
        critic.add(Conv3D(1, kernel_size=(1, 1, 1), activation='relu', kernel_initializer='he_uniform', input_shape=state_size, padding='valid'))
        critic.add(Flatten())    
        critic.add(Dense(64, activation='relu'))
        critic.add(Dropout(dropout))
        ...
        actor=Sequential()
        actor.add(Conv3D(1, kernel_size=(1, 1, 1), activation='relu', kernel_initializer='he_uniform', input_shape=state_size, padding='valid'))
        #Input(shape=[1])
        actor.add(Flatten())
        actor.add(Dense(64, activation='relu'))
        actor.add(Dropout(dropout))
        actor.add(Dense(self.action_size, activation='softmax'))
        #model = Model(inputs=state_input, outputs=output)
        #model = Model(input=[state_input, advantage], output=[output])
        actor.compile(optimizer=Adam(lr=LR_actor), loss='categorical_crossentropy')
    ...
    def train(self):
        ...
        while True:
            ...
            pylab.savefig("./save_graph/a3c.png")

            self.save_model('./save_model/a3c.h5')
