...
class environment(gym.Env):
    ...
    def __init__(self, x,y,z, cutoff, turnspc, policy, rg_prob='loadenv', rendermode='off', envpath='./environments/cutoff'):
        ...
        self.cutoff=cutoff #modify loaded environment categorising anything below cutoff as waste.
    ...
    def save(self, savenum='count'):
        ...
        if savenum =='count':
           ...
           self.savenumber=np.random.randint(1,self.maxloadid)
    ...
    def load(self, loadid):
        ...
        a=self.geo_array[:,:,:,0]
        b=self.geo_array[:,:,:,1]
        a=np.where(a>self.cutoff, a, 0.0)
        a=a.reshape([self.Ilen, self.Jlen, self.RLlen,1])
        b=b.reshape([self.Ilen, self.Jlen, self.RLlen,1])
        self.geo_array=np.append(a, b, axis=3)
        #self.geo_array=np.array([a,b])
    ...
    def evaluate(self, selected_block, isMinable, isEfficient):
        ...
        if isMinable==0:             #penalising repetetive useless actions
            ...
            ore=-self.penaltyscalar*self.averagereward
    ...
    def reset(self):
        ...
        #self.turns=min(round((len(self.dep_dic)*(self.dturnspc*self.episodecounter/self.annealrate+self.startingturnspc))),round((len(self.dep_dic)*(self.dturnspc+self.startingturnspc)))) 
        # self.dturnspc+self.startingturnspc = final turnspc
        self.terminal=False
        self.i=-1
        self.j=-1
        self.RL=-1
