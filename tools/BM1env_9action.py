...
from tools.createmodel import auto1model
...
class environment(gym.Env):
    ...
    def __init__(self, x,y,z ,gamma, turnspc, policy, rg_prob='rg', rendermode='off'):
       ...
       # self.cutoffpenaltyscalar=penaltyscalar #scaling parameter for changing the penalty for taking no action (cutoff).
        ...
        self.channels = 1#H2O mean, mined state, Standard deviation
        self.flatlen=self.Ilen*self.Jlen*self.RLlen*self.channels
        self.i=round(self.Imax/2)
        self.j=round(self.Jmax/2)
        ...
        #create block model
        self.model=auto1model(self.Ilen,self.Jlen,self.RLlen)
        self.build()
        ...
        self.action_space = spaces.Discrete(9)#+1) #+1 action for choosing terminal state.
    ...
    def actcoords(self, action):
        ...
        if action==0:
            ...
            if isMinable==True:
                ...
                pass#self.reward=-self.averagereward*stepcostfactor 
    ...
    def equip_failure(self):
        ...
        if random.random()>0.9998**self.turncounter: #probability of success
            self.terminal=True
        else:
            self.terminal=False
        return
    ...
    def step(self, action):        
        ...
        if sum(sum(sum(self.ob_sample[:,:,:])))>=self.ob_sample[:,:,:].size: #if all blocks are mined, end episode
            self.terminal=True
        ...
        elif action>=(9): #end episode if terminal action 9 is taken (not currently enabled)
            self.terminal=True
        ...
        if self.policy=='MlpPolicy':
            arr=np.ndarray.flatten(self.ob_sample) #flatten model for MLP (not CNN) policy
            observation=arr.reshape([len(arr)])
    ...
    def update(self, selected_block):
        ...
        self.ob_sample[self.i,self.j,self.RL]=0 #set to one (mined) for agent observation.
