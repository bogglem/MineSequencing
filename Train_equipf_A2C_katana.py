...
#cutoffpenaltyequipf=inputarray.loc[idx].cutoffpenaltyequipf #not currently implemented
...
equipf_s=str(equipf).replace('.','_')

#cutoff_s=str(cutoffpenaltyequipf).split('.')[0]
#rg_s=rg_prob #max(str(float(rg_prob)).split('.'))
turnspc_s=str(turnspc).split('.')[1]
storagefolder='output'
scenario=str(f'{trialv}_{inputfile_s}_t{test}_lr{LR_s}_g{gamma_s}_s{equipf_s}')    
...
def make_env(x,y,z, rank, seed=0):
    ...
    def _init():
        ...
        env = environment(x, y, z, gamma, turnspc, equipf, policyname)
        env.seed(seed + rank)
        return env
...
if __name__ == '__main__':
    ...
    env1 =environment(x, y, z, gamma, turnspc, equipf, policyname) #env annealreate/ numturns*eval_freq
